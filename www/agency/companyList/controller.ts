...
export function CompanyListController($scope, $loading, Models) {
    $("title").html("公司列表");
    //企业管理首页信息
...
export function CompanyDetailController($scope, $stateParams, $loading, Models) {
    $("title").html("公司详情");
...
//创建公司页面
export function CreateCorpController($scope) {
...
//员工管理页面
export function StaffListController($scope, $stateParams) {
    var companyId = $stateParams.company;
    $scope.companyId = companyId;
    $(".left_nav li").removeClass("on").eq(1).addClass("on");
...
//组织架构页面
export function DepartmentController($scope, $stateParams, $loading) {
    $("title").html("组织架构");
    //初始化
 ...
 * @constructor
 */
export function PolicyListController($scope, $stateParams, $loading) {
    $("title").html("差旅标准");
    //Myselect();
    $scope.companyId = $stateParams.companyId;
    ...
    $scope.initPolicyList = function() {
        ...
        API.onload(function() {
            var params = {} as any;
            var options = {order: [["created_at", "asc"]]} as any;
            options.perPage = 100;
            params.options = options;
