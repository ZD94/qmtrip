"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : new P(function (resolve) { resolve(result.value); }).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t;
    return { next: verb(0), "throw": verb(1), "return": verb(2) };
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (_) try {
            if (f = 1, y && (t = y[op[0] & 2 ? "return" : op[0] ? "throw" : "next"]) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [0, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
Object.defineProperty(exports, "__esModule", { value: true });
var path = require("path");
var API = require('@jingli/dnode-api');
function wechatUploaderController($scope, $element, $loading, ngModalDlg, wxApi) {
    var config = require('@jingli/config');
    $element.click(function () {
        return __awaiter(this, void 0, void 0, function () {
            var localIds, serverIds, _i, localIds_1, id, serverId, files, urls, e_1;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        _a.trys.push([0, 8, 9, 10]);
                        return [4 /*yield*/, wxApi.chooseImage({
                                count: 1,
                                sizeType: ['original', 'compressed'],
                                sourceType: ['album', 'camera'],
                            })];
                    case 1:
                        localIds = _a.sent();
                        serverIds = [];
                        _i = 0, localIds_1 = localIds;
                        _a.label = 2;
                    case 2:
                        if (!(_i < localIds_1.length)) return [3 /*break*/, 5];
                        id = localIds_1[_i];
                        return [4 /*yield*/, wxApi.uploadImage({
                                localId: id,
                                isShowProgressTips: 1,
                            })];
                    case 3:
                        serverId = _a.sent();
                        serverIds.push(serverId);
                        _a.label = 4;
                    case 4:
                        _i++;
                        return [3 /*break*/, 2];
                    case 5:
                        $loading.start();
                        return [4 /*yield*/, API.wechat.mediaId2key({ mediaIds: serverIds })];
                    case 6:
                        files = _a.sent();
                        return [4 /*yield*/, config.$ready];
                    case 7:
                        _a.sent();
                        urls = files.map(function (f) {
                            return path.join(config.update, 'attachment/temp', f.fileId) + '?expireTime=' + f.expireTime + '&sign=' + f.sign;
                        });
                        $scope.uploader.addToQueue(urls);
                        return [3 /*break*/, 10];
                    case 8:
                        e_1 = _a.sent();
                        console.log(e_1.stack);
                        $scope.done()({
                            code: -1,
                            errMsg: '文件上传失败'
                        });
                        return [3 /*break*/, 10];
                    case 9:
                        $loading.end();
                        return [7 /*endfinally*/];
                    case 10: return [2 /*return*/];
                }
            });
        });
    });
}
exports.wechatUploaderController = wechatUploaderController;
