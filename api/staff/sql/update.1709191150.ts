...
export = async function(db, transaction) {
    ...
    for(let i =0; i < companies.length; i++ ){
        ...
        let isEmpty = await db.query(`select count(*) as total from staff.staffs where company_id = '${companies[i].id}' and travel_policy_id is null and deleted_at is null;`, {type: Sequelize.QueryTypes.SELECT});
         if(!isEmpty || isEmpty.length == 0 || !isEmpty[0].total || isEmpty[0].total == 0) continue;

        let defaultTravelPolicySql = `select * from travel_policy.travel_policies where company_id = '${companies[i].id}' and  deleted_at is null;`;
        ...
        if(defaultTravelPolicy && defaultTravelPolicy.length) {
            let updateStaff = `update staff.staffs set travel_policy_id = '${defaultTravelPolicy[0].id}', updated_at = now() where company_id = '${companies[i].id}' and travel_policy_id is null;`;
            try{
