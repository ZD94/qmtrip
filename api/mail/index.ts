...
import {EmailQueue} from "../../JLTypes/_types/mail/email";
    ...
    scheduler("*/10 * * * * *", "sendEmail", function() {
        (async () => {
            ...
            var promises = emails.map(async function(email){
                ...
                try{
                    //查询邮件主题内容
                    let submits = await Models.emailSubmit.find({where: {id: email.submitId}});
...
async function _recordEmailLog(id, status, error) {
    ...
    return DB.transaction(async function(t) {
        try{
            ...
            var emailLog = {id: email.id, submitId: email["emailSubmitId"],
            ...
            let update = await Promise.all([
                ...
                await email.destroy(),
                async function(){
                    let emailLog = EmailLog.create(emailLog);
                    await emailLog.save();
                }()
            ])
            return update;
...
export class Mail{
    ...
    async sendMailRequest(params) {
        ...
        for(var i=0, ll: number =emails.length; i<ll; i++) {
            var queue = {id: uuid.v1(), submitId: submit.id, toUser: emails[i], maxSendTimes: 1};
