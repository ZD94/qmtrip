...
tripPlan.deleteConsumeDetail = function (params) {
    ...
    return ConsumeDetails.findById(id, {attributes: ['accountId']})
        .then(function (detail) {
            ...
            return ConsumeDetails.update({status: -2, updateAt: utils.now()}, {
                where: {id: id},
                fields: ['status', 'updateAt']
            })
        })
        .then(function () {
            return true;
...
tripPlan.uploadInvoice = function (params) {
    ...
    return ConsumeDetails.findOne({where: {id: params.consumeId, account_id: params.userId}})
        ...
        .then(function (list) {
            ...
            return PlanOrder.update({status: 1, updateAt: utils.now()}, {
                where: {id: orderId},
                fields: ['status', 'updateAt'],
                returning: true
            })
            ...
            return sequelize.transaction(function (t) {
                return Q.all([
                    ...
                    .spread(function (ret) {
                        ...
                        return ConsumeDetails.findAll({
                                where: {orderId: order.id, status: {$ne: -2}},
                            ...
                            .then(function (isAllAudit) {
                                ...
                                return PlanOrder.update(order_updates, {
                                    where: {id: order.id},
                                    fields: fields,
                                    transaction: t
                                })
