...
export default async function update(DB: Sequelize, t: Transaction){
    ...
    await Promise.all(tripPlans.map(async (tripPlan: TripPlan) => {
        ...
        await Promise.all(tripDetails.for(async (tripDetail: TripDetail) => {
        ...
        switch(status) {
            ...
            case -3:  //EOldPlanStatus.AUDIT_NOT_PASS
                ...
                await DB.query(updateSql);
                updateSql = `update trip_plan.trip_details set "reserve_status" = where id = '${tripPlan.id}'`;
                await DB.query(updateSql);
                updateSql = '';
                break;
            ...
            case 1:   //EOldPlanStatus.WAIT_UPLOAD
                if(!reservedExists) {  //未预定，
                    if(tripPlan.backAt < new Date()) {   //未预定，且超时，设置为报销单
                        updateSql = `update trip_plan.trip_plans set status = 7, "auditStatus" = 3 where id = '${tripPlan.id}';`; //更新失效，待上传, 
                        await DB.query(updateSql);
                        updateSql = `update trip_plan.trip_details set reserveStatus = -3 where trip_plan_id = '${tripPlan.id}'`; //未预定，tripDetail的reserveStatus设置未待预定，status维持报销单状态不变，
                        await DB.query(updateSql);
                        updateSql = '';
                    } else { ////未预定，未超时，设置为待预定
                        updateSql = `update trip_plan.trip_plans set status = 5, "auditStatus" = -4 where id = '${tripPlan.id}';`;
                        await DB.query(updateSql);
                        updateSql = `update trip_plan.trip_details set status = 6, reserveStatus = -3 where trip_plan_id = '${tripPlan.id}'`; //未预定，tripDetail的reserveStatus设置未待预定，status为待预定，
                        await DB.query(updateSql);
                        updateSql = '';
                    }
